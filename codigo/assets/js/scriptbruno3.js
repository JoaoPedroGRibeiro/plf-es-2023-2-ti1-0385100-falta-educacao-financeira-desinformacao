$(document)
  .on("change", ".dataput", function () {
    ...
    if (vencem) {
      ...
      let daysDiff =
        (compDateParse - currentParse) / (1000 * 60 * 60 * 24 * 30);
      $(".prazo").val(daysDiff.toFixed(2));
    } else if (prazoAno) $(".prazo").val((parseFloat(prazoAno) * 12).toFixed(2));
  })
  ...
  .on("click", ".simulaUm", function () {
    ...
    for (let i = 1; i <= prazo; i++) {
      ...
      acuPush =
        Math.floor(
          (acuPush + parseInt((tempJuros + aporte) * 100) / 100) * 100
        ) / 100;
      console.log(
        "anterior: " + acuPush + " juros: " + tempJuros + " aporte " + aporte
      );
      ...
      jurosPush =
        Math.floor(parseFloat(acuPush - aportePush).toFixed(2) * 100) / 100;
      ...
      acumulado +=
        ...
        i +
        "</td><td>" +
        Number(aportePush).praPessoa() +
        "</td><td>" +
        Number(tempJuros).praPessoa() +
        "</td><td>" +
        Number(jurosPush).praPessoa() +
        "</td><td>" +
        Number(acuPush).praPessoa() +
        "</td></tr>";
    }
    if (prazo === parseInt(prazo)) console.log("inteiro");
...
Calculator.Calculate = function () {
  var s = [],
    ...
    t;
  if (
    (l && (c = $("input[name=surfaceType]:checked").val()),
    (u = $("#dropTypePaint").val()),
    (f = $("#dropHands").val()),
    !l)
  ) {
    MessageBox.Show(
      "Erro!",
      'Selecione uma opção para o campo "Tipo de Superfície"!'
    );
    return;
  }
  if (String.IsNullOrEmpty(u) || u == "-1") {
    MessageBox.Show(
      "Erro!",
      'Selecione uma opção para o campo "Tipo de produto"!'
    );
    return;
  }
  if (((e = $("input[name=hdnDimensionsWallIndex]")), e.length == 0)) {
    MessageBox.Show(
      "Erro!",
      'Preencha uma opção para o campo "Dimensões a ser pintada"!'
    );
    return;
  }
  for (t = 0; t < e.length; t++) {
    ...
    if (!Validator.IsDecimal($("#txtHeight_" + n).val()) || i == 0) {
      MessageBox.Show(
        "Erro!",
        'Valor informado para o campo "Altura da dimensão a ser pintada" inválido!'
      );
      return;
    }
    if (!Validator.IsDecimal($("#txtWidth_" + n).val()) || r == 0) {
      MessageBox.Show(
        "Erro!",
        'Valor informado para o campo "Largura da dimensão a ser pintada" inválido!'
      );
      return;
    }
    s.push({
  ...
  for (
    o = $("input[name=hdnDimensionsDoorWindowIndex]"), t = 0;
    t < o.length;
    t++
  ) {
    var n = o[t].value,
      i = parseFloat(
        $("#txtHeightDoorWindow_" + n)
          .val()
          .replace(",", ".")
      ),
      r = parseFloat(
        $("#txtWidthDoorWindow_" + n)
          .val()
          .replace(",", ".")
      );
    if (
      (i != 0 && Validator.IsDecimal($("#txtHeightDoorWindow_" + n).val())) ||
      (r != 0 && Validator.IsDecimal($("#txtWidthDoorWindow_" + n).val()))
    ) {
      if (
        !Validator.IsDecimal($("#txtHeightDoorWindow_" + n).val()) ||
        i == 0
      ) {
        MessageBox.Show(
          "Erro!",
          'Valor informado para o campo "Altura da Porta / Janela" inválido!'
        );
        return;
      }
      if (!Validator.IsDecimal($("#txtWidthDoorWindow_" + n).val()) || r == 0) {
        MessageBox.Show(
          "Erro!",
          'Valor informado para o campo "Largura da Porta / Janela" inválido!'
        );
        return;
      }
      h.push({
  ...
  if (String.IsNullOrEmpty(f) || f == "-1") {
    MessageBox.Show(
      "Erro!",
      'Selecione uma opção para o campo "Número de demão"!'
    );
    return;
  }
